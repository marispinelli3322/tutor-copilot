...
export interface ModuleGlossary {
  id: string;
  title: string;
  entries: GlossaryEntry[];
}

export function getGlossaryData(locale: Locale): ModuleGlossary[] {
  if (locale === "en") return glossaryEN;
  return glossaryPT;
}

export function getModuleGlossary(locale: Locale, moduleId: string): ModuleGlossary | undefined {
  const data = getGlossaryData(locale);
  return data.find((m) => m.id === moduleId);
}
